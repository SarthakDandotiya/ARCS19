...
(function(window) {
	...
	Photostack.prototype._showPhoto = function(pos) {
		...
		classie.addClass(this.currentItem, "photostack-inactive", 5000);
	...
	Photostack.prototype._shuffle = function(resize) {
		...
		var overlapFactor = 0.5,
			// lines & columns
			lines = Math.ceil(
				this.sizes.inner.width / (this.sizes.item.width * overlapFactor)
			),
			columns = Math.ceil(
				this.sizes.inner.height /
					(this.sizes.item.height * overlapFactor)
			),
			// since we are rounding up the previous calcs we need to know how much more we are adding to the calcs for both x and y axis
			addX =
				lines * this.sizes.item.width * overlapFactor +
				this.sizes.item.width / 2 -
				this.sizes.inner.width,
			addY =
				columns * this.sizes.item.height * overlapFactor +
			...
			moveItems = function() {
				...
				for (var i = 0; i < columns; ++i) {
					...
					for (var j = 0; j < lines; ++j) {
						var xVal =
								j * (self.sizes.item.width * overlapFactor) -
								extraX,
							yVal =
								i * (self.sizes.item.height * overlapFactor) -
								extraY,
							olx = 0,
							oly = 0;
				...
				self.allItems.forEach(function(item, i) {
					...
					if (
						self.items.indexOf(item) === self.current &&
						self.started &&
						iter === 0
					) {
						...
						self.currentItem.style.msTransform =
							...
							self.centerItem.y +
							"px) rotate(0deg)";
						self.currentItem.style.transform =
							"translate(" +
							self.centerItem.x +
							"px," +
							self.centerItem.y +
						...
						// if there is something behind..
						if (
							self.currentItem.querySelector(".photostack-back")
						) {
						...
						item.style.WebkitTransform =
							...
							"px) rotate(" +
							Math.floor(
								Math.random() * (maxrot - minrot + 1) + minrot
							) +
							"deg)";
	...
	Photostack.prototype._isOverlapping = function(itemVal) {
		var dxArea = this.sizes.item.width + this.sizes.item.width / 5, // adding some extra avoids any rotated item to touch the central area
			dyArea = this.sizes.item.height + this.sizes.item.height / 5,
			areaVal = {
		...
		if (
			...
			var left = Math.random() < 0.5,
				...
				noOverlapX = left
					? (itemVal.x - areaVal.x + dxItem) * -1 - randExtraX
					: areaVal.x +
					  dxArea -
					  (itemVal.x + dxItem) +
					  dxItem +
					  randExtraX,
				noOverlapY = left
					? (itemVal.y - areaVal.y + dyItem) * -1 - randExtraY
					: areaVal.y +
					  dyArea -
					  (itemVal.y + dyItem) +
					  dyItem +
