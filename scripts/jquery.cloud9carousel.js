...
(function($) {
	...
	var Item = function(element, options) {
		...
		this.moveTo = function(x, y, scale) {
			...
			if (transform && options.transforms) {
				...
				if (options.mirror && this.element.tagName === "IMG")
					this.reflection.style.marginTop =
						options.mirror.gap * scale + "px";
	...
	var Carousel = function(element, options) {
		...
		this.items = [];
		this.xOrigin =
			options.xOrigin === null
				? $container.width() * 0.5
				: options.xOrigin;
		this.yOrigin =
			options.yOrigin === null
				? $container.height() * 0.1
				: options.yOrigin;
		this.xRadius =
			options.xRadius === null
				? $container.width() / 2.3
				: options.xRadius;
		this.yRadius =
			options.yRadius === null
				? $container.height() / 6
				: options.yRadius;
		...
		this.renderItem = function(itemIndex, rotation) {
			...
			item.moveTo(
				this.xOrigin +
					scale *
						(Math.cos(rotation) * this.xRadius -
							item.fullWidth * 0.5),
		...
		this.itemsRotated = function() {
			return (
				(this.items.length * (Math.PI / 2 - this.rotation)) /
				(2 * Math.PI)
			);
		};
